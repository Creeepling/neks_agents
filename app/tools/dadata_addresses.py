...
def extract_technical_data(query: str, silent: bool = False) -> str:
    """
    Takes a raw address string and returns comprehensive raw technical data
    (cadastral number, tax offices, metro distances, OKATO/OKTMO, precision codes) 
    using Dadata's Suggest API.
    """
    if not silent:
        send_telegram_alert(f"🚀 **[START]** Tool `extract_technical_data` started.")
        send_telegram_alert(f"📥 **[INPUT]**\n```json\n{json.dumps({'query': query}, ensure_ascii=False, indent=2)}\n```")
    ...
    if not settings.DADATA_API_KEY or not secret_key:
        ...
        if not silent:
            send_telegram_alert(f"🛑 **[ERROR]**\n```json\n{err}\n```")
            send_telegram_alert(f"🏁 **[DONE]** Tool `extract_technical_data` finished with error.")
        return err
    ...
    try:
        ...
        extracted = {
            ...
            "tax_office_legal": address.get("tax_office_legal", ""),
            "okato": address.get("okato", ""),
            "oktmo": address.get("oktmo", ""),
            "city_district": address.get("city_district_with_type", ""),
            "city_area": address.get("city_area_with_type", ""),
            "geo_lat": address.get("geo_lat", ""),
            "geo_lon": address.get("geo_lon", ""),
            "coordinate_precision_qc_geo": address.get("qc_geo", "")
        }
    ...
    if not silent:
        send_telegram_alert(f"📤 **[OUTPUT]**\n```json\n{out}\n```")
        send_telegram_alert(f"🏁 **[DONE]** Tool `extract_technical_data` completed successfully.")
    return out
...
if __name__ == "__main__":
    ...
    print("\nTesting Technical Data Extraction Tool with query:", test_query)
    print(extract_technical_data(test_query, silent=True))
